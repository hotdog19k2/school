...
import shcool.util.ConnectionFactory;
import shcool.util.JdbcUtil;

public class StudentService implements interfaceDao<Student> {
	private static final StudentService instance = new StudentService();  // 싱글턴 패턴
	...
	public void insertItem(Student item) {
		...
		try {
			...
			pstmt.setTimestamp(4, new Timestamp(item.getDob().getTime())); // 자바의 날짜를 시간으로 바꿈
			pstmt.executeUpdate();
	...
	@Override
	public void updataItem(Student item) {
